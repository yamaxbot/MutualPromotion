...
async def start_sq():
    ...
    cur.execute("CREATE TABLE IF NOT EXISTS clients(id TEXT, point TEXT, subscription TEXT, referals TEXT, whose_referals TEXT)")
...
async def add_all_clients_sql(id, whose_referals='No'):
    cur.execute("INSERT INTO clients VALUES(?, ?, ?, ?, ?)", (id, 0, '0', '0', whose_referals))
    db.commit()
...
async def add_points_sql(id):
    ...
    point = point[1]
    cur.execute("UPDATE clients SET point = ? WHERE id = ?", (float(point)+1, id))
    db.commit()
...
async def add_two_points_sql(id):
    point = cur.execute("SELECT * FROM clients WHERE id = ?", (id,)).fetchone()
    point = point[1]
    cur.execute("UPDATE clients SET point = ? WHERE id = ?", (float(point)+2, id))
    db.commit()
...
async def return_points_sql(id, point_p):
    point = cur.execute("SELECT * FROM clients WHERE id = ?", (id,)).fetchone()
    point = point[1]
    cur.execute("UPDATE clients SET point = ? WHERE id = ?", (float(point) + int(point_p), id))
    db.commit()
...
async def issue_points_sql(id, point):
    ...
    points = float(client[1]) + float(point)
...
async def bust_my_active_order_sql(id):
    data = cur.execute("SELECT * FROM fast_orders WHERE customer = ?", (str(id),)).fetchall()
    for d in data:
        cur.execute("DELETE FROM fast_orders WHERE number = ?", (d[0],))
        cur.execute("INSERT INTO fast_orders VALUES(?, ?, ?, ?, ?, ?)", (d[0], d[1], d[2], d[3], d[4], d[5],))
    db.commit()
