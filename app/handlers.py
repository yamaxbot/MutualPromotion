...
async def command_start_handler(message: Message, state: FSMContext):
    await state.clear()
    await message.answer('⚡️Добро пожаловать в Mutual Promotion\n\n📌В этом сервисе, вы можете создавать задания на гарантированное выполнение в любой социальной сети:\n• 👥 Подписки\n• ❤️ Реакции\n• 📝 Комментарии\n• 🤖 Продвижение ботов\n\n📜Узнать инструкцию можно введя команду /help\n\n❓Если возникли вопросы пишите в поддержку: @Mutual_Promotion2_Bot', reply_markup=kb.client_reply_keyboards)
...
async def use_promo_two_handler(message: Message, state: FSMContext):
    ...
    for promo in all_promo:
        ...
        if message.text == promo[1] and str(message.from_user.id) not in use_user and int(promo[2]) > 0:
            ...
            break
    if flag == False:
        await message.answer('⚠Возникла ошибка из за следующих причин:\n\n❗️-Промокод уже неактивен\n\n❗️-Вы уже вводили данный промокод\n\n❗️-Такой промокод не существует')
        await state.clear()
